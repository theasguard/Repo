...
class Orionoid:
	...
	priority = 0
	pack_capable = False # Ezra requires separate functions to scrape packs. Just retrieve packs as part of the normal scrape and label them as packs.
	hasMovies = True
	hasEpisodes = True
	...
	Keys = {
		'default' : 'VWtOQ1NVbEZkMmRVUTBKSVNVVnZaMU5EUWxaSlJsbG5WR2xDUWtsRk1HZFNlVUpOU1VacloxUnBRa2xKUmtWblZWTkNSVWxFWjJkV1EwSkhTVVZaWjA5RFFURkpSbEZuVmxOQk5VbEdSV2RTVTBKRw==',
		'plugin.video.ezra' : 'VldsQ1NFbEZZMmRPVTBKUFNVVlpaMU5EUWtWSlJrRm5Ua05DV1VsRlNXZFdlVUpTU1VWSloxTjVRa2hKUkdkblVsTkNVMGxGTUdkVmFVRTFTVVZ2WjFGcFFsRkpSVWxuVVhsQ1EwbEZVV2RVVTBKTA==',
	}

	def __init__(self, type):
		self.type = type
		self.addon = xbmcaddon.Addon('script.module.ezscrapers')
	...
	def _error(self):
		...
		xbmc.log('EZRA SCRAPERS ORION [ERROR]: ' + parameters, xbmc.LOGERROR)
		...
		try:
			...
			for data in results:
				try:
					...
					# Do not add the 'package' attribute, otherwise Ezra estimates the file size per episode, and Orion already does that.
					# if data['file']['pack'] and type == Orion.TypeShow: item['package'] = 'season'

					if details: item['name_info'] = details
					else: item['name_info'] = ' ' # Not an empty string, since there is a bug in Ezra.
