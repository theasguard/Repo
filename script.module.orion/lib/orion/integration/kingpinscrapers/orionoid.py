...
from kingpinscrapers.modules import source_utils
...
class Orionoid:
	...
	priority = 0
	pack_capable = False # KingPin requires separate functions to scrape packs. Just retrieve packs as part of the normal scrape and label them as packs.
	hasMovies = True
	hasEpisodes = True
	...
	Keys = {
		'default' : 'VW5sQ1ZVbEdSV2RTVTBKTFNVWkJaMVZUUWtOSlJXTm5VbWxDV0VsR1FXZFNhVUUxU1VWTloxSnBRa05KUlZsblZVTkJNMGxGTUdkUFUwSkZTVVZuWjFSRFFraEpSazFuVDFOQ1VVbEZZMmRVUTBKSA==',
		'plugin.video.kingpin' : 'VW1sQk1FbEZWV2RTUTBKQ1NVVlZaMUo1UVRWSlJYZG5VbWxDUkVsRmQyZFBRMEpOU1VaQloxUkRRbEZKUlc5blRrTkNUMGxGVFdkVFEwSk1TVVZ2WjFScFFsVkpSa0ZuVTNsQ1JrbEVhMmRVUTBKUA==',
	}

	def __init__(self, type):
		self.type = type
		self.addon = xbmcaddon.Addon('script.module.kingpinscrapers')
	...
	def _error(self):
		...
		xbmc.log('KINGPIN SCRAPERS ORION [ERROR]: ' + parameters, xbmc.LOGERROR)
		...
		try:
			...
			for data in results:
				try:
					...
					# Magnet links require a hash attribute, otherwise KingPin throws an exception an fails.
					...
					else: item['name_info'] = ' ' # Not an empty string, since there is a bug in KingPin.
