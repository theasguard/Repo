...
def toggle_auto_play():
    if __autoplay_enabled():
        ...
        kodi.notify(msg='Asguard Auto-Play Turned Off')
    else:
        addon.setSetting('auto-play', 'true')
        kodi.notify(msg='Asguard Auto-Play Turned On')
...
def set_related_url(mode, media_type, title, year):
    ...
    runstring = 'RunPlugin(plugin://plugin.video.asguard%s)' % (kodi.get_plugin_url(queries))
    xbmc.executebuiltin(runstring)
...
def scraper_sort_order():
    ...
    runstring = 'RunAddon(plugin.video.asguard,%s)' % (kodi.get_plugin_url(queries))
    xbmc.executebuiltin(runstring)
    
def addon_settings():
    xbmc.executebuiltin('Addon.OpenSettings(plugin.video.asguard)')
...
def __is_blamo_listitem(li_path):
    ...
    try:
        lines = xbmcvfs.File(li_path).read()
        if lines and not lines.startswith('plugin://plugin.video.asguard'):
...
def main(argv=None):
    ...
    if __is_blamo_listitem(path):
        ...
        except: ret = dialog.select('Asguard Tools', [i[0] for i in tools])
        if ret > -1:
            tools[ret][1](*tools[ret][2])
    else:
        kodi.notify(msg='Not a Asguard Library Item')
