...
BASE_URL = 'https://revelationmedia.tk'
XML_META = {
    '4keyecandy.xml': {'quality': QUALITIES.HD1080, 'format': '4K'},
    '4kmovies.xml': {'quality': QUALITIES.HD1080, '3D': True},
    'chocolatemain.xml': {'quality': QUALITIES.HD720}
}
    ...
    def search(self, video_type, title, year, season=''):  # @UnusedVariable
        ...
        for page_url in folders:
            ...
            for match in re.finditer('<item>(.*?)</item>', xml, re.I | re.DOTALL):
                ...
                match_url = re.search('<sublink>(.*?)</sublink>', item, re.I)
                if match_title_year and match_url:
                    ...
                    if match_title_year and match_url:
                        ...
                        match_url = 'xml_file=%s&sublink=%s' % (xml_file, match_url)
