...
BASE_URL = 'http://dl.funsaber.net/serial/'
...
class Scraper(scraper.Scraper):
    ...
    def get_sources(self, video):
        ...
        if video.video_type == VIDEO_TYPES.TVSHOW:
            meta = scraper_utils.parse_episode_link(source_url)
    ...
    def search(self, video_type, title, year, season=''):  # @UnusedVariable
        ...
        if video_type == VIDEO_TYPES.TVSHOW:
            results = self.__show_search(title, year)
        else:
    ...
    def __show_search(self, title, year):
        results = []
        norm_title = scraper_utils.normalize_title(title)
        html = self._http_get(self.base_url, cache_limit=48)
        for item in scraper_utils.parse_directory(self, html):
            if not item['directory']:
                meta = scraper_utils.parse_episode_link(item['title'])
