...
class Scraper(scraper.Scraper):
    ...
    def __init__(self, timeout=scraper.DEFAULT_TIMEOUT):
        ...
        logger.log('Comet: Initializing scraper', log_utils.LOGDEBUG)
        self._set_apikeys()
        ...
        logger.log('Comet: Scraper initialized successfully', log_utils.LOGDEBUG)
    ...
    def _build_config_params(self):
        """Build Comet configuration parameters with user's debrid API keys"""
        import base64
        
        # Configuration matching Comet's expected format
        config = {
        ...
        if self.rd_apikey:
            ...
            logger.log('Comet: Using Real-Debrid API key', log_utils.LOGDEBUG)
        elif self.tb_apikey:
            config["debridApiKey"] = self.tb_apikey
            config["debridService"] = "torbox"
            debrid_configured = True
            logger.log('Comet: Using TorBox API key', log_utils.LOGDEBUG)
        elif self.ad_apikey:
            config["debridApiKey"] = self.ad_apikey
            config["debridService"] = "alldebrid"
            debrid_configured = True
            logger.log('Comet: Using AllDebrid API key', log_utils.LOGDEBUG)
        elif self.pm_apikey:
            config["debridApiKey"] = self.pm_apikey
            config["debridService"] = "premiumize"
            debrid_configured = True
            logger.log('Comet: Using Premiumize API key', log_utils.LOGDEBUG)
        elif self.dl_apikey:
            config["debridApiKey"] = self.dl_apikey
            config["debridService"] = "debridlink"
            debrid_configured = True
            logger.log('Comet: Using DebridLink API key', log_utils.LOGDEBUG)
        
        if not debrid_configured:
            # Fallback to default TorBox key
            config["debridApiKey"] = kodi.get_setting('torbox.apikey')
            config["debridService"] = "torbox"
            logger.log('Comet: No user debrid API keys configured, using default TorBox key', log_utils.LOGWARNING)
        
        # Encode configuration to base64
        config_json = json.dumps(config, separators=(',', ':'))
        params = base64.b64encode(config_json.encode('utf-8')).decode('utf-8')
        
        logger.log('Comet: Generated config for debrid service: %s' % config["debridService"], log_utils.LOGDEBUG)
        return params
    ...
    def get_sources(self, video):
        ...
        try:
            logger.log('Comet: Starting get_sources for video type: %s' % video.video_type, log_utils.LOGDEBUG)
            ...
            logger.log('Comet: Found IMDB ID: %s' % imdb_id, log_utils.LOGDEBUG)
            ...
            logger.log('Comet: Search URL template: %s' % search_url, log_utils.LOGDEBUG)

            url = urllib.parse.urljoin(self.base_url, search_url)
            logger.log('Comet: Request URL: %s' % url, log_utils.LOGDEBUG)
            
            response = self._http_get(url, cache_limit=1, require_debrid=True)
            if not response or response == FORCE_NO_MATCH:
                logger.log('Comet: No response from server for URL: %s' % url, log_utils.LOGWARNING)
                return sources
            
            logger.log('Comet: Received response length: %d characters' % len(response), log_utils.LOGDEBUG)

            try:
                data = json.loads(response)
                files = data.get('streams', [])
                logger.log('Comet: Found %d streams in response' % len(files), log_utils.LOGDEBUG)
                
                if not files:
                    logger.log('Comet: No streams found. Response keys: %s' % list(data.keys()), log_utils.LOGDEBUG)
                    if 'error' in data:
                        logger.log('Comet: API error: %s' % data['error'], log_utils.LOGWARNING)
                    
            except json.JSONDecodeError as e:
                logger.log('Comet: Failed to parse JSON response: %s' % str(e), log_utils.LOGERROR)
                logger.log('Comet: Response content: %s' % response[:500], log_utils.LOGDEBUG)
                return sources
            ...
            for file in files:
                try:
                    ...
                    if not file_url:
                        logger.log('Comet: Skipping stream without URL', log_utils.LOGDEBUG)
                        continue
                        
                    logger.log('Comet: Processing stream: %s' % file_url[:100], log_utils.LOGDEBUG)
                    
                    # Comet uses 'description' field for the stream title
                    name = file.get('description', file.get('title', 'Unknown'))
                    
                    # Extract quality from the name
                    quality = scraper_utils.get_tor_quality(name)
                    
                    # Extract size and other info from Comet's description format
                    info = []
                    size = 0
                    
                    # Comet might include size in different formats
                    size_match = re.search(r'💾\s*([\d.]+)\s*(GB|MB)', name) or re.search(r'(\d+(?:\.\d+)?)\s*(GB|MB)', name)
                    if size_match:
                    ...
                    # Look for seeders/peers information
                    seeders_match = re.search(r'👥\s*(\d+)', name) or re.search(r'(\d+)\s*seed', name, re.IGNORECASE)
                    if seeders_match:
                        seeders = seeders_match.group(1)
                        info.append('%s seeds' % seeders)
                    
                    # Look for tracker information
                    tracker_match = re.search(r'⚙️\s*([^\s💾👥🎬📀🔊]+)', name)
                    if tracker_match:
                        tracker = tracker_match.group(1)
                        info.append(tracker)

                    source_info = ' | '.join(info) if info else 'Comet'
                    
                    # Clean up the title for display (remove emoji metadata)
                    ...
                    logger.log('Comet: Added source: %s [%s] [%s]' % (clean_name, quality, source_info), log_utils.LOGDEBUG)
