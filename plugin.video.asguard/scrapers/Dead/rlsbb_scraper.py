...
from asguard_lib import debrid, client
...
class Scraper(scraper.Scraper):
    ...
    def get_sources(self, video):
        ...
        logging.debug("rlsbb Source URL: %s", source_url)
        hosters = []
        sources = {}
    ...
    def __get_post_links(self, html, video):
        sources = {}
        post = dom_parser2.parse_dom(html, 'div', {'class': 'post-content-wrapper'})
        if post:
            post = post[0].content
    ...
    def search(self, video_type, title, year, season=''):  # @UnusedVariable
        ...
        headers = {'Referer': referer + urllib.parse.quote_plus(title)}
        headers.update(XHR)
        query = title + ' '
        search_url = scraper_utils.urljoin(SEARCH_BASE_URL, '/?s=' + urllib.parse.quote_plus(query))
        logger.log(f'Searching for: {query} at {search_url}', log_utils.LOGDEBUG)
        ...
        for post in js_data.get('results', []):
            ...
            result = self._blog_proc_results(post.get('post_title', ''), '(?P<post_title>.+)(?P<url>.*?)', '', video_type, title)
            if result:
