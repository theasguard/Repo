...
from asguard_lib.utils2 import SHORT_MONS, VIDEO_TYPES, i18n
from asguard_lib import cloudflare, scraper_utils, control
...
class Scraper(scraper.Scraper):
    ...
    def get_sources(self, video):
        ...
        if source_url and source_url != FORCE_NO_MATCH:
            ...
            try:
                html = self._http_get(url, headers=headers, require_debrid=False, cache_limit=.5)
                print(html)
    ...
    def _get_episode_url(self, show_url, video):
        ...
        while page_url and not too_old:
            ...
            print(url)
            headers = {'User-Agent': LOCAL_UA}
            html = self._http_get(url, headers=headers, require_debrid=False, cache_limit=.1)
            print(html)
    ...
    def search(self, video_type, title, year, season=''):
        ...
        if video_type == VIDEO_TYPES.TVSHOW and title:
            test_url = '/?s=%s/' % (self.__to_slug(title))
            print(test_url)
            test_url = urllib.parse.urljoin(self.base_url, test_url)
            print(test_url)
            headers = {'User-Agent': LOCAL_UA}
            try:
                html = self._http_get(test_url, headers=headers, require_debrid=True, cache_limit=.4)
                print(html)
            except Exception as e:
                logger.log(f"Error fetching URL {test_url}: {(e)}", log_utils.LOGERROR)
                return results
            ...
            print(headers)
            try:
                html = self._http_get(search_url, require_debrid=True, cache_limit=.4)
                print(html)
            except Exception as e:
                logger.log(f"Error fetching URL {search_url}: {(e)}", log_utils.LOGERROR)
                return results
