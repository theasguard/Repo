...
SEARCH_URL = '/?search%5Btree%5D=~cat0_z3&sortdir=DESC&sortby=stamp&search%5Bvalue%5D%5B%5D=Title%3A%3D%3ADEF%3A{query}'
...
class Scraper(scraper.Scraper):
    ...
    def get_sources(self, video):
        ...
        logger.log(f'ClubNZB: Source URL: {source_url}', log_utils.LOGDEBUG)
        if not source_url or source_url == FORCE_NO_MATCH:
            return sources

        html_data = self._http_get(source_url, require_debrid=True, cache_limit=1)
        logger.log(f'ClubNZB: HTML response length: {len(html_data) if html_data else 0}', log_utils.LOGDEBUG)
        if not html_data:
            logger.log('ClubNZB: No data returned', log_utils.LOGWARNING)
            return sources
        ...
        try:
            ...
            # Find the table with class "spots"
            spots_table = soup.find('table', class_='spots')
            if not spots_table:
                logger.log('ClubNZB: No spots table found', log_utils.LOGWARNING)
                return sources

            # Get all rows except header
            rows = spots_table.find_all('tr')[1:]  # Skip header row
            ...
            for row in rows:
                try:
                    ...
                    # Cell 1: Category (skip)
                    # Cell 2: Title
                    title_cell = cells[1]
                    title_link = title_cell.find('a')
                    name = title_link.text.strip() if title_link else title_cell.text.strip()

                    # Cell 3: Comments count (skip)
                    # Cell 4: Category info (skip)
                    # Cell 5: Poster (skip)
                    # Cell 6: Age (skip)
                    # Cell 7: Size
                    size_text = cells[6].text.strip()
                    size_bytes = self._parse_size(size_text)

                    # Cell 8: NZB download link
                    nzb_cell = cells[7]
                    nzb_link = nzb_cell.find('a', class_='nzb')
                    if not nzb_link:
                        continue

                    url = nzb_link.get('href', '')
                    if not url.startswith('http'):
                        url = f'{self.base_url}{url}'

                    # Extract message ID from URL for logging
                    message_id = re.search(r'messageid=([^&]+)', url)
                    logger.log(f'ClubNZB: Item: {message_id.group(1) if message_id else "unknown"}, {name}, {size_text}', log_utils.LOGDEBUG)
                    ...
                    if self.min_size > 0 and size_bytes < (self.min_size * 1024 * 1024):
                        logger.log(f'ClubNZB: Skipping {name} - size {size_bytes} bytes below minimum {self.min_size} MB', log_utils.LOGDEBUG)
                        continue
                    ...
                    logger.log(f'ClubNZB: Found NZB: {name} - {size_text}', log_utils.LOGDEBUG)

                except Exception as e:
                    logger.log(f'ClubNZB: Error parsing row: {str(e)}', log_utils.LOGWARNING)
                    continue

            logger.log(f'ClubNZB: Found {len(sources)} sources', log_utils.LOGDEBUG)

        except Exception as e:
            logger.log(f'ClubNZB: Error parsing response: {str(e)}', log_utils.LOGERROR)

        return self._filter_sources(sources, video)

    def _parse_size(self, size_text):
        """Convert size string (e.g., '28.93 GB') to bytes"""
        try:
            size_text = size_text.strip().upper()
            if 'GB' in size_text:
                return float(size_text.replace('GB', '').strip()) * 1024**3
            elif 'MB' in size_text:
                return float(size_text.replace('MB', '').strip()) * 1024**2
            elif 'KB' in size_text:
                return float(size_text.replace('KB', '').strip()) * 1024
            elif 'TB' in size_text:
                return float(size_text.replace('TB', '').strip()) * 1024**4
            else:
                return 0
    ...
    def _filter_sources(self, sources, video):
        """
        Filters NZB sources based on anime-specific naming patterns and season/episode matching.

        Args:
            sources (list): List of NZB sources containing 'name' and metadata
        ...
        for source in sources:
            ...
            for season_num in possible_season_numbers:
                if any([
                    ...
                    f"seizoen {season_num:02d}" in name, # seizoen 01 (Dutch)
                    f"seizoen {season_num}" in name      # seizoen 1 (Dutch)
                ]):
                    matches_season = True
                    break
            ...
            if any(keyword in name for keyword in season_pack_keywords):
                ...
                if (matches_season or
                    ...
                    is_season_pack = True
                    logger.log(f'ClubNZB: Valid season pack found: {name}', log_utils.LOGDEBUG)
                else:
                    logger.log(f'ClubNZB: Batch/complete label found but wrong season: {name}', log_utils.LOGDEBUG)
            ...
            if is_season_pack or (matches_season and (not has_episode_info or matches_episode)):
                ...
                logger.log(f'ClubNZB: Filtered source: {source}', log_utils.LOGDEBUG)
    ...
    def has_any_season_indicator(self, name):
        ...
        patterns = [
            ...
            r'seizoen\s*\d{1,2}'  # seizoen 1 (Dutch)
        ]
        return any(re.search(p, name) for p in patterns)

    def episode_in_range(self, name, target_ep):
        """Check for episode ranges like 01-24, 001~112, or 01 to 24"""
        match = re.search(r'(\d{2,4})[-~](\d{2,4})', name) or re.search(r'(\d{2,4})\s*to\s*(\d{2,4})', name)
        if match:
    ...
    def get_url(self, video):
        if video.video_type == VIDEO_TYPES.MOVIE:
            ...
            # For TV shows, search with title only
            query = f'{video.title}'

        # Remove colons and other special characters that might cause URL encoding issues
        query = query.replace(':', '').replace(';', '')

        return f'{self.base_url}{SEARCH_URL.format(query=quote_plus(query))}'
