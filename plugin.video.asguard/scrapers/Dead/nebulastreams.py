...
class Scraper(scraper.Scraper):
    ...
    def get_sources(self, video):
        ...
        try:
            ...
            if not imdb_id:
                logger.log('NebulaStreams: No IMDB ID found for trakt_id: %s' % video.trakt_id, log_utils.LOGWARNING)
                return sources

            if video.video_type == VIDEO_TYPES.MOVIE:
                search_path = self.movie_search_url % imdb_id
                logger.log('NebulaStreams: Searching for movie: %s' % imdb_id, log_utils.LOGDEBUG)
            elif video.video_type == VIDEO_TYPES.EPISODE:
                search_path = self.tv_search_url % (imdb_id, video.season, video.episode)
                logger.log('NebulaStreams: Searching for episode: %s S%sE%s' % (imdb_id, video.season, video.episode), log_utils.LOGDEBUG)
            else:
                logger.log('NebulaStreams: Unsupported video type: %s' % video.video_type, log_utils.LOGWARNING)
                return sources
            ...
            logger.log('NebulaStreams: Response: %s' % response, log_utils.LOGDEBUG)
            if not response:
                logger.log('NebulaStreams: No response from server', log_utils.LOGWARNING)
                return sources

            try:
                data = json.loads(response)
                streams = data.get('streams', [])
                logger.log('NebulaStreams: Found %d streams' % len(streams), log_utils.LOGDEBUG)
            except json.JSONDecodeError as e:
                logger.log('NebulaStreams: Failed to parse JSON response: %s' % str(e), log_utils.LOGERROR)
                return sources
            ...
            for stream in streams:
                try:
                    ...
                    logger.log('NebulaStreams: Processing: %s' % display_name, log_utils.LOGDEBUG)
                    ...
                    logger.log('NebulaStreams: Found source: %s [%s]' % (display_name, source_info), log_utils.LOGDEBUG)

                except Exception as e:
                    logger.log('NebulaStreams: Error processing stream: %s' % str(e), log_utils.LOGWARNING)
                    continue

        except Exception as e:
            logger.log('NebulaStreams: Unexpected error in get_sources: %s' % str(e), log_utils.LOGERROR)

        logger.log('NebulaStreams: Returning %d sources' % len(sources), log_utils.LOGDEBUG)
        return sources
    ...
    def search(self, video_type, title, year, season=''):
        """
        Search method implementation for NebulaStreams scraper.
        NebulaStreams requires IMDB IDs, so search functionality is limited.
        """
        logger.log('NebulaStreams: Search not implemented - requires IMDB ID', log_utils.LOGDEBUG)
        return []
