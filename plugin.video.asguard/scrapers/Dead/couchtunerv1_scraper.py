...
BASE_URL = 'https://www.couchtuner.show'
BASE_URL2 = 'https://freecouchtuner.com'
...
class Scraper(scraper.Scraper):
    ...
    def get_sources(self, video):
        ...
        for _attribs, tab in dom_parser2.parse_dom(entry, 'div', {'class': 'postTabs_divs'}):
            ...
            hoster = {'multi-part': False, 'host': host, 'class': self, 'quality': scraper_utils.get_quality(video, host, QUALITIES.HIGH), 'views': None, 'rating': None, 'url': link, 'direct': False, 'debridonly': False}
    ...
    def search(self, video_type, title, year, season=''):  # @UnusedVariable
        show_list_url = scraper_utils.urljoin(self.base_url, '/free-watch-series/')
    ...
    def get_settings(cls):
        ...
        name = cls.get_name()
        settings.append(f'         <setting id="{name}-base_url" type="text" label="     {i18n("base_url")}" default="{BASE_URL}" visible="true"/>')
        return settings
