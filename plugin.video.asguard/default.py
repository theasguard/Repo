...
def main_menu():
    ...
    if TOKEN:
        ...
        logger.log('Trakt profile: %s' % (profile), log_utils.LOGDEBUG)
...
def settings_menu():
    # Add Trakt user info header
    if TOKEN:
        try:
            profile = trakt_api.get_user_profile()
            user_info = '[COLOR cyan]%s[/COLOR] ([COLOR grey]%s[/COLOR]) [COLOR deeppink]VIP %s[/COLOR]' % (
            ...
            kodi.create_item(
                {'mode': 'noop'}, 
                'Trakt Account: %s' % user_info,
                thumb=utils2.art('settings.png'),
                fanart=utils2.art('fanart.jpg'),
                is_folder=False,
                is_playable=False
            )
        except Exception as e:
            logger.log('Failed to get Trakt profile: %s' % e, log_utils.LOGWARNING)

    # Add Trakt auth button
    auth_label = '[COLOR yellow]%s[/COLOR]' % utils2.i18n('trakt_acct_auth')
    if not TOKEN:
        auth_label += ' [COLOR red](%s)[/COLOR]' % utils2.i18n('not_authenticated')
        
    kodi.create_item(
        {'mode': MODES.AUTH_TRAKT}, 
        auth_label,
        thumb=utils2.art('settings.png'),
        fanart=utils2.art('fanart.jpg'),
        is_folder=False, 
    ...
    kodi.create_item({'mode': MODES.AUTH_TRAKT}, i18n('trakt_acct_auth'), thumb=utils2.art('settings.png'), fanart=utils2.art('fanart.jpg'), is_folder=False, is_playable=False)
    ...
    kodi.create_item({'mode': MODES.SETTINGS}, 'This addon developed and supported by MrBlamo', thumb=utils2.art('settings.png'), fanart=utils2.art('fanart.jpg'))
    kodi.set_content(CONTENT_TYPES.ADDONS)
    kodi.end_of_directory()
...
def browse_menu(section):
    ...
    if utils2.menu_on('trending'):
        kodi.create_item({'mode': MODES.TRENDING, 'section': section}, i18n('trending').format(section_label), thumb=utils2.art('trending.png'), fanart=utils2.art('fanart.jpg'))
    if utils2.menu_on('popular'):
        kodi.create_item({'mode': MODES.POPULAR, 'section': section}, i18n('popular').format(section_label), thumb=utils2.art('popular.png'), fanart=utils2.art('fanart.jpg'))
    if utils2.menu_on('anticipated'):
        kodi.create_item({'mode': MODES.ANTICIPATED, 'section': section}, i18n('anticipated').format(section_label), thumb=utils2.art('anticipated.png'), fanart=utils2.art('fanart.jpg'))
    if utils2.menu_on('recent'):
        kodi.create_item({'mode': MODES.RECENT, 'section': section}, i18n('recently_updated').format(section_label), thumb=utils2.art('recent.png'), fanart=utils2.art('fanart.jpg'))
    if utils2.menu_on('mosts'):
    ...
    if TOKEN:
        if utils2.menu_on('on_deck'):
            kodi.create_item({'mode': MODES.SHOW_BOOKMARKS, 'section': section}, i18n('trakt_on_deck'), thumb=utils2.art('on_deck.png'), fanart=utils2.art('fanart.jpg'))
        if utils2.menu_on('recommended'):
            kodi.create_item({'mode': MODES.RECOMMEND, 'section': section}, i18n('recommended').format(section_label), thumb=utils2.art('recommended.png'), fanart=utils2.art('fanart.jpg'))
        if utils2.menu_on('collection'):
            add_refresh_item({'mode': MODES.SHOW_COLLECTION, 'section': section}, i18n('my_collection').format(section_label), utils2.art('collection.png'), utils2.art('fanart.jpg'))
        if utils2.menu_on('history'):
            kodi.create_item({'mode': MODES.SHOW_HISTORY, 'section': section}, i18n('watched_history'), thumb=utils2.art('watched_history.png'), fanart=utils2.art('fanart.jpg'))
        if utils2.menu_on('favorites'):
            kodi.create_item({'mode': MODES.SHOW_FAVORITES, 'section': section}, i18n('my_favorites'), thumb=utils2.art('my_favorites.png'), fanart=utils2.art('fanart.jpg'))
        if utils2.menu_on('subscriptions'):
            add_refresh_item({'mode': MODES.MANAGE_SUBS, 'section': section}, i18n('my_subscriptions'), utils2.art('my_subscriptions.png'), utils2.art('fanart.jpg'))
        if utils2.menu_on('watchlist'):
            add_refresh_item({'mode': MODES.SHOW_WATCHLIST, 'section': section}, i18n('my_watchlist'), utils2.art('my_watchlist.png'), utils2.art('fanart.jpg'))
        if utils2.menu_on('my_lists'):
            kodi.create_item({'mode': MODES.MY_LISTS, 'section': section}, i18n('my_lists'), thumb=utils2.art('my_lists.png'), fanart=utils2.art('fanart.jpg'))
        if utils2.menu_on('liked_lists'):
            add_refresh_item({'mode': MODES.LIKED_LISTS, 'section': section}, i18n('liked_lists'), utils2.art('liked_lists.png'), utils2.art('fanart.jpg'))
    if utils2.menu_on('other_lists'):
        kodi.create_item({'mode': MODES.OTHER_LISTS, 'section': section}, i18n('other_lists'), thumb=utils2.art('other_lists.png'), fanart=utils2.art('fanart.jpg'))
    if section == SECTIONS.TV:
        if TOKEN:
            if utils2.menu_on('progress'):
                add_refresh_item({'mode': MODES.SHOW_PROGRESS}, i18n('my_next_episodes'), utils2.art('my_progress.png'), utils2.art('fanart.jpg'))
            if utils2.menu_on('rewatch'):
                add_refresh_item({'mode': MODES.SHOW_REWATCH}, i18n('my_rewatches'), utils2.art('my_rewatch.png'), utils2.art('fanart.jpg'))
            if utils2.menu_on('my_cal'):
                add_refresh_item({'mode': MODES.MY_CAL}, i18n('my_calendar'), utils2.art('my_calendar.png'), utils2.art('fanart.jpg'))
        if utils2.menu_on('general_cal'):
    ...
    if utils2.menu_on('search'):
        add_search_item({'mode': MODES.RECENT_SEARCH, 'section': section}, i18n('recent_searches'), utils2.art(section_params['search_img']), MODES.CLEAR_RECENT)
    if utils2.menu_on('search'):
        add_search_item({'mode': MODES.SAVED_SEARCHES, 'section': section}, i18n('saved_searches'), utils2.art(section_params['search_img']), MODES.CLEAR_SAVED)
    if OFFLINE:
        kodi.notify(msg='[COLOR blue]***[/COLOR][COLOR red] %s [/COLOR][COLOR blue]***[/COLOR]'.format(i18n('trakt_api_offline')))
    kodi.set_content(CONTENT_TYPES.ADDONS)
    kodi.end_of_directory()
...
def show_views():
    content_types = ['movies', 'tvshows', 'seasons', 'episodes', 'files']
    for content_type in content_types:
        label = i18n('set_default_x_view') % (content_type.capitalize())
        thumb = utils2.art('settings.png')
        fanart = utils2.art('fanart.jpg')
        kodi.create_item({'mode': MODES.BROWSE_VIEW, 'content_type': content_type}, label, thumb=thumb, fanart=fanart)
    kodi.end_of_directory()
...
def install_themepak():
    xbmc.executebuiltin('RunPlugin(plugin://script.asguard.themepak)')
...
def scraper_settings():
    ...
    for i, cls in enumerate(scrapers):
        ...
        if failures == -1:
            ...
            index = int(min([(failures * (len(COLORS) - 1) // fail_limit), len(COLORS) - 1]))
    ...
    if not slug:
        ...
        try:
            ...
            for rewatch in rewatches:
                ...
                menu_items = []
                queries = {'mode': MODES.SEASONS, 'trakt_id': trakt_id, 'title': show['title'], 'year': show['year'], 'tvdb_id': show['ids']['tvdb'], 'tmdb_id': show['ids']['tmdb']}
                menu_items.append((i18n('browse_seasons'), 'Container.Update(%s)' % (kodi.get_plugin_url(queries))),)
...
def browse_seasons(trakt_id, title, year, tvdb_id=None, tmdb_id=None):
    ...
    for season in seasons:
        if kodi.get_setting('show_season0') == 'true' or season['number'] != 0:
            ...
            liz = make_season_item(season, season_info, trakt_id, title, year, tvdb_id, tmdb_id)
...
def make_group_episode_item(show_title, year, trakt_id, tmdb_id, episode, season, ep_watched):
    ...
    logger.log('Group Episode info: %s' % episode_info, log_utils.LOGDEBUG)
...
@url_dispatcher.register(MODES.TRAKT_EPISODE_GROUPS, ['tmdb_id', 'trakt_id'], ['title', 'tvdb_id'])
def browse_episode_groups(tmdb_id, trakt_id, title='', tvdb_id=None):
    logger.log('Browsing episode groups for tmdb_id=%s, trakt_id=%s' % (tmdb_id, trakt_id), log_utils.LOGDEBUG)
    
    tmdb_seasons = tmdb_api.get_tv_episode_groups(tmdb_id)
    logger.log('TMDB Episode Groups: %s' % tmdb_seasons, log_utils.LOGDEBUG)
    
    if not tmdb_seasons:
        logger.log('No episode groups found', log_utils.LOGNOTICE)
        kodi.notify(msg='No episode groups found', duration=5000)
        return
        
    # Filter groups to show most relevant types first
    # Type 6 is "Original Order"
    valid_groups = sorted(tmdb_seasons, key=lambda g: 0 if g.get('type') == 6 else g.get('type', 999))
    ...
    for group in valid_groups:
        ...
        type_label = {
            ...
            6: 'Production', 
            7: 'TV'
        }.get(group_type, 'Unknown')
        
        # Create a label with the group type in brackets
        label = f"{group_name} [{type_label}]"
        logger.log('Group Label: %s' % label, log_utils.LOGDEBUG)
        
        # Get group details to display groups
        group_data = tmdb_api.get_episode_group_details(group_id)
        if group_data and 'groups' in group_data:
            for idx, season_group in enumerate(group_data['groups']):
                # Create season listing
                overview = season_group.get('overview') or season_group.get('description', '')
                ids = {'trakt': trakt_id, 'tvdb': tvdb_id, 'tmdb': tmdb_id}
                season_number = season_group.get('order', idx)
                season_label = f"{label} - {season_group.get('name', '')}"
                meta = salts_utils.make_info({'title': title, 'overview': overview}, ids, season_label)
...
@url_dispatcher.register(MODES.EPISODE_GROUPS, ['group_id', 'season'], ['trakt_id', 'title', 'tvdb_id', 'trakt'])
def browse_group_episodes(group_id, season, trakt_id=None):
    logger.log('Browsing group episodes for group_id=%s, season=%s, trakt_id=%s' % (group_id, season, trakt_id), log_utils.LOGDEBUG)
    # Get watched history if logged in to Trakt
    watched = {}
    if TOKEN:
        ...
        for item in watched_history:
            ...
            for s in item['seasons']:
                watched[show_id][s['number']] = set(e['number'] for e in s['episodes'])
    ...
    for group in group_data['groups']:
        logger.log('Group: %s' % group, log_utils.LOGDEBUG)
        if group.get('order', -1) == int(season):
            # Ensure we're iterating over episode dictionaries
            ...
            logger.log('Total Group episodes: %s' % totalItems, log_utils.LOGDEBUG)

            # Get all unique show IDs from episodes to batch fetch show details
            show_ids = set()
            for ep in episodes:
            ...
            # Batch fetch show details
            show_details_map = {}
            if show_ids:
                show_details_map = tmdb_api.get_tv_details_batch(list(show_ids))

            for ep in episodes:
                logger.log('Main Group Episode: %s' % ep, log_utils.LOGDEBUG)

                # Use actual season number from episode data instead of group season
                actual_season = ep.get('season_number')
                if actual_season is None:
                    actual_season = int(season)  # Fallback to group season if not available
                
                ep_watched = False
                if trakt_id and trakt_id in watched:
                    if actual_season in watched[trakt_id]:
                        ep_watched = ep['episode_number'] in watched[trakt_id][actual_season]
                
                overview = ep.get('overview')
                ep_num = ep.get('order', 0) + 1
                year = ep.get('air_date').split('-')[0]
                tmdb_id = ep.get('show_id')
                tmdb_group_details = show_details_map.get(str(tmdb_id), {})
                if tmdb_group_details:
                    title = tmdb_group_details.get('name')
                    external_ids = tmdb_group_details.get('external_ids')
                    ...
                    xbmcplugin.addDirectoryItem(HANDLE, liz_url, li, isFolder=False, totalItems=totalItems)
...
def apply_urlresolver(hosters):
    ...
    logger.log('debrid_resolvers: %s' % debrid_resolvers, log_utils.LOGDEBUG)
    filtered_hosters = []
    debrid_hosts = {}
    unk_hosts = {}
    known_hosts = {}
...
def play_source(mode, hoster_url, direct, video_type, trakt_id, season='', episode=''):
    ...
    with kodi.WorkingDialog() as wd:
        ...
        try:
            if video_type == VIDEO_TYPES.EPISODE:
                ...
                show_meta = trakt_api.get_show_details(trakt_id) or {}
                logger.log('Playback Show Meta: %s' % show_meta, log_utils.LOGDEBUG)
                tvdb_id = show_meta.get('ids', {}).get('tvdb')
                
                try:
                    ep_meta = trakt_api.get_episode_details(trakt_id, season, episode)
                    logger.log('Trakt episode: %s' % ep_meta, log_utils.LOGDEBUG)
                except TraktNotFoundError as e:
                    # Try TVDB as fallback

                    from asguard_lib.tvdb_api.tvdb_api import TheTvDb
                    tvdb_api = TheTvDb()
                    episodes = tvdb_api.get_episodes_by_season(tvdb_id, season)
                    logger.log('TVDB episodes: %s' % episodes, log_utils.LOGDEBUG)

                    # Find the specific episode in the season list
                    tvdb_ep = None
                    for ep in episodes:
                        if str(ep.get('airedEpisodeNumber')) == str(episode):
                            tvdb_ep = ep
                            break
                    
                    logger.log('TVDB play Episode: %s' % tvdb_ep, log_utils.LOGDEBUG)
                
                    if tvdb_ep:
                        # Map TVDB data to Trakt-like structure
                        ep_meta = {
                            'season': season,
                            'number': episode,
                            'title': tvdb_ep.get('episodeName', ''),
                            'original_title': tvdb_ep.get('episodeName', ''),
                            'ids': {'tvdb': tvdb_ep.get('id', '')},
                            'overview': tvdb_ep.get('overview', ''),
                            'rating': tvdb_ep.get('rating', 0),
                            'votes': tvdb_ep.get('ratingCount', 0),
                            'first_aired': tvdb_ep.get('aired', '')
                        }
                        logger.log('Using TVDB fallback for episode data', log_utils.LOGDEBUG)
                    else:
                        logger.log('TVDB episode not found', log_utils.LOGWARNING)
                except Exception as e_tvdb:
                    logger.log(f'TVDB API error: {str(e_tvdb)}', log_utils.LOGERROR)
            
                    if not ep_meta:
                        logger.log('Using minimal metadata fallback', log_utils.LOGWARNING)
                        ep_meta = trakt_api.get_episode_details(trakt_id, season, episode)
                ...
                logger.log('Make Playback: People: %s' % (people), log_utils.LOGDEBUG)
    ...
    with kodi.WorkingDialog() as wd:
        ...
        set_info_tag(listitem, info, 'video', 
                    old_method_keys=('size', 'count', 'date',))
...
def pick_source_dir(prev_mode, hosters, video_type, trakt_id, season, episode):
    ...
    logger.log('Building source directory for: %s' % (queries), log_utils.LOGDEBUG)
    plugin_url = kodi.get_plugin_url(queries)
...
def flush_cache():
    ...
    yes = i18n('keep')
    no = i18n('delete')
    # Convert localized strings to integers for yesno dialog if needed
    yes_label = 1 if yes.lower() == 'yes' else 0
    no_label = 1 if no.lower() == 'yes' else 0
    if dlg.yesno(i18n('flush_web_cache'), ln1, ln2, ln3, yes_label, no_label):
        with kodi.WorkingDialog() as wd:
            start = None
            monitor = xbmc.Monitor()  # Use Monitor for Kodi 21 compatibility
            while not monitor.abortRequested():
                days_left = db_connection.prune_cache(prune_age=0)
                if start is None: start = days_left
                if days_left:
                    wd.update_progress(100 * (start - days_left) / start)
                else:
...
def flush_image_cache():
    ...
    yes_label = 1 if yes.lower() == 'yes' else 0
    no_label = 1 if no.lower() == 'yes' else 0
    if dlg.yesno(i18n('flush_image_cache'), ln1, ln2, ln3, yes_label, no_label):
        with kodi.WorkingDialog():
...
def prune_cache():
    ...
    while not monitor.abortRequested():
        if xbmc.getInfoLabel('Container.PluginName') != kodi.get_id():
            ...
            logger.log('Asguard Active... Busy... Postponing [%s]' % (MODES.PRUNE_CACHE), log_utils.LOGDEBUG)
...
def reset_db():
    try:
        ...
        logger.log('Database reset failed: %s' % str(e), log_utils.LOGERROR)
        if 'file may be in use' in str(e).lower() or 'cannot access the file' in str(e).lower():
            message = 'Database reset failed: File is in use by other processes. Please close Kodi completely and try again.'
        else:
            message = 'Database reset failed: %s' % str(e)
    kodi.notify(msg=message, duration=8000)
...
def make_dir_from_list(section, list_data, slug=None, query=None, page=None):
    logger.log('Making directory from list: %s' % (list_data), log_utils.LOGDEBUG)
    section_params = utils2.get_section_params(section)
    watched = {}
    in_collection = {}
    if TOKEN:
...
def make_season_item(season, info, trakt_id, title, year, tvdb_id, tmdb_id):
    logger.log('Make Season: Season: %s, Info: %s, Trakt ID: %s, Title: %s, Year: %s, TVDB ID: %s, TMDB ID: %s' % (season, info, trakt_id, title, year, tvdb_id, tmdb_id), log_utils.LOGDEBUG)
    label = '%s %s' % (i18n('season'), season['number'])
    ids = {'trakt': trakt_id, 'tvdb': tvdb_id, 'tmdb': tmdb_id}
    # cached_ids = db_connection.cache_tmdb_trakt_mapping(tmdb_id, trakt_id)
    # logger.log('Cached IDs: %s' % (cached_ids), log_utils.LOGDEBUG)
...
def make_episode_item(show, episode, show_subs=True, menu_items=None):
    ...
    try: 
        time_str = time.asctime(time.localtime(utc_air_time))
    except: 
...
def make_item(section_params, show, menu_items=None):
    logger.log('make_item: %s' % show, log_utils.LOGDEBUG)
    if menu_items is None: menu_items = []
    ...
    liz.setProperty('resume', str(0))
    liz.setProperty('total', str(1))
    return liz, liz_url
...
def tmdb_search(query=None):
    if query is None:
        ...
        if keyboard.isConfirmed():
            ...
            kodi.notify(msg=i18n('search_cancelled'), duration=5000)
            return
...
def tmdb_tv_search():
    ...
    if not query:
        kodi.notify(msg="No search query entered.", duration=5000)
        return

    # Display a progress dialog while searching
    progress = xbmcgui.DialogProgress()
    progress.create("Searching TMDB", "Searching for TV shows...")
    progress.update(25, "Fetching results from TMDB...")
    
    # Use the multi-page search to get more results (up to 60 shows)
    shows = tmdb_api.search_tmdb_tv_multi_page(query, max_pages=3)
    
    progress.update(75, "Processing results...")
    ...
    shows = sorted(shows, key=lambda x: x.get('popularity', 0), reverse=True)
    
    progress.close()
    
    totalItems = len(shows)
    for show in shows:
        liz, liz_url = makeitem.make_tv_show_item(show)
        xbmcplugin.addDirectoryItem(int(sys.argv[1]), liz_url, liz, isFolder=True, totalItems=totalItems)
    kodi.set_view(CONTENT_TYPES.TVSHOWS, True)
    kodi.end_of_directory()

@url_dispatcher.register(MODES.TMDB_SEASONS, ['tmdb_id'])
def browse_tmdb_seasons(tmdb_id):
    show = tmdb_api.get_tv_details(tmdb_id)
    
    # Add Episode Groups as first item
    groups = tmdb_api.get_tv_episode_groups(tmdb_id)
    if groups:
        liz = utils.make_list_item('[B]Episode Groups[/B]', {'type': 'groups'})
        liz.setArt({'icon': os.path.join(kodi.get_path(), 'resources', 'episode_groups.png'),
                   'thumb': os.path.join(kodi.get_path(), 'resources', 'episode_groups.png')})
        liz_url = kodi.get_plugin_url({'mode': MODES.TMDB_EPISODES, 'tmdb_id': tmdb_id, 'season': -1})
        xbmcplugin.addDirectoryItem(int(sys.argv[1]), liz_url, liz, isFolder=True)


    if not isinstance(show, dict):
        logger.log('Expected show to be a dictionary, but got: {}'.format(type(show)), log_utils.LOGERROR)
        return
    
    seasons = tmdb_api.get_tv_seasons(tmdb_id)
    totalItems = len(seasons)
    for season in seasons:
        liz, liz_url = makeitem.make_tmdb_season_item(season, show, tmdb_id)
        xbmcplugin.addDirectoryItem(int(sys.argv[1]), liz_url, liz, isFolder=True, totalItems=totalItems)
    kodi.set_view(CONTENT_TYPES.SEASONS, True)
    kodi.end_of_directory()


@url_dispatcher.register(MODES.TMDB_EPISODES, ['tmdb_id', 'season'])
def browse_tmdb_episodes(tmdb_id, season):
    show = tmdb_api.get_tv_details(tmdb_id)
    
    # Handle episode groups when season = -1
    if int(season) == -1:
        groups = tmdb_api.get_tv_episode_groups(tmdb_id)
        logger.log('TMDB Episode Groups: %s' % groups, log_utils.LOGDEBUG)
        ...
        for group in valid_groups:
            ...
            if group_data and 'groups' in group_data:
                for idx, season_group in enumerate(group_data['groups']):
                    ...
                    # Create a custom episode object for makeitem
                    episode_obj = {
                        'episode_group': True, 
                        'id': group_id,
                        'name': season_label,
                    ...
                    liz, liz_url = makeitem.make_tmdb_episode_item(
                        {'name': show.get('name', ''), 'id': tmdb_id},  # Pass show details
                        episode_obj,
                        tmdb_id
                    )
                    xbmcplugin.addDirectoryItem(int(sys.argv[1]), liz_url, liz, isFolder=True)
    ...
    episodes = tmdb_api.get_season_episodes(tmdb_id, season)
    totalItems = len(episodes)
    now = time.time()
    for episode in episodes:
        ...
        if kodi.get_setting('show_unaired') == 'true' or utc_air_time <= now:
            if kodi.get_setting('show_unknown') == 'true' or utc_air_time:
                liz, liz_url = makeitem.make_tmdb_episode_item(show, episode, tmdb_id)
                if liz and liz_url:
                    xbmcplugin.addDirectoryItem(int(sys.argv[1]), liz_url, liz, isFolder=False, totalItems=totalItems)
    kodi.set_view(CONTENT_TYPES.EPISODES, True)
    kodi.end_of_directory()
...
def add_contextsearchmenu(title, video_type):
    ...
    xbmcplugin.setContent(addon, 'movies')
