...
def request(url, close=True, redirect=True, error=False, proxy=None, post=None, headers=None, mobile=False, XHR=False, limit=None, referer=None, cookie=None, compression=True, output='', timeout='30'):
    try:
        ...
        if output in ['cookie', 'extended'] or not close:
        ...
        if 'User-Agent' not in headers:
            headers['User-Agent'] = cache.get(randomagent, 1) if not mobile else 'Apple-iPhone/701.341'
        if 'Referer' not in headers:
            headers['Referer'] = '%s://%s/' % (urllib.parse.urlparse(url).scheme, urllib.parse.urlparse(url).netloc) if referer is None else referer
        if 'Accept-Language' not in headers:
            headers['Accept-Language'] = 'en-US'
        if 'X-Requested-With' not in headers and XHR:
            headers['X-Requested-With'] = 'XMLHttpRequest'
        if 'Cookie' not in headers and cookie is not None:
            headers['Cookie'] = cookie
        if 'Accept-Encoding' not in headers and compression and limit is None:
        ...
        try:
            ...
            if response.code == 503:
                ...
                try:
                    ...
                    encoding = None
                if encoding == 'gzip':
                    cf_result = gzip.GzipFile(fileobj=six.BytesIO(cf_result)).read()
...
class cfcookie:
    ...
    def get(self, netloc, ua, timeout):
        ...
        for i in range(0, 15):
            threads.append(worker_pool.Thread(self.get_cookie, netloc, ua, timeout))
