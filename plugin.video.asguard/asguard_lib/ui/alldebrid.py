...
AGENT = 'Asguard for Kodi'
VERSION = kodi.get_addon_version()
USER_AGENT = '{0}/{1}'.format(AGENT, VERSION)
FORMATS = kodi.supported_video_extensions()
...
class AllDebridResolver(ResolveUrl):
    ...
    def get_media_url(self, host, media_id, cached_only=False, return_all=False):
        try:
                ...
                if r:
                    ...
                    if self.__check_cache(_hash):
                        ...
                        if self.get_setting('cached_only') == 'true' or cached_only:
                            raise Exception('AllDebrid: {0}'.format(i18n('cached_torrents_only')))
                        else:
                            transfer_id = self.__create_transfer(_hash)
            ...
            try:
                ...
                msg = 'Unknown Error (2)'
            raise Exception('AllDebrid Error: {0} ({1})'.format(msg, e.code))
        else:
            js_result = json.loads(result)
            logger.log_debug('AllDebrid resolve: [{0}]'.format(js_result))
            if 'error' in js_result:
                e = js_result.get('error')
                raise Exception('AllDebrid Error: {0} ({1})'.format(e.get('message'), e.get('code')))
                ...
                if js_result.get('data').get('link'):
                    ...
                    sid = utils2.pick_source(utils2.sort_sources_list(sources))
        ...
        raise Exception('AllDebrid: {0}'.format(i18n('no_stream')))
    ...
    def __check_cache(self, media_id):
        ...
        if result.get('status') == "success":
            ...
            if ecode == "AUTH_BLOCKED":
                ...
                raise Exception(i18n('validate'))
            elif ecode == "AUTH_USER_BANNED":
                logger.log_debug('Exception during AD auth: {0}'.format(ecode))
                raise Exception(i18n('banned'))
            return False
    ...
    def __initiate_transfer(self, transfer_id, interval=5):
        try:
            ...
            if transfer_info:
                ...
                with kodi.ProgressDialog('Asguard AllDebrid {0}'.format(i18n('transfer')), line1, line2, line3) as pd:
                    while not transfer_info.get('statusCode') == 4:
                        ...
                        if pd.is_canceled():
                            keep_transfer = kodi.yesnoDialog(
                                heading='AsGuard AllDebrid {0}'.format(i18n('transfer')),
                                line1=i18n('ad_background')
                            )
                            if not keep_transfer:
                                self.__delete_transfer(transfer_id)
                            raise Exception('{0} ID {1} :: {2}'.format(i18n('transfer'), transfer_id, i18n('user_cancelled')))
                        elif 5 <= transfer_info.get('statusCode') <= 10:
                            self.__delete_transfer(transfer_id)
                            raise Exception('{0} ID {1} :: {2}'.format(i18n('transfer'), transfer_id, transfer_info.get('status')))

                xbmc.sleep(1000 * interval)  # allow api time to generate the links

            return

        except Exception as e:
            self.__delete_transfer(transfer_id)
            raise Exception('Transfer ID {0} :: {1}'.format(transfer_id, e))
    ...
    def authorize_resolver(self):
        ...
        with kodi.CountdownDialog(
            'Asguard AllDebrid {0}'.format(i18n('authorisation')), line1, line2,
