...
from asguard_lib import client, jsunpack
from asguard_lib.pyaes import AESModeOfOperationCBC, Decrypter, Encrypter
...
def load_video_from_url(in_url):
    ...
    if found_extractor is None:
        logging.error("[*E*] No extractor found for %s" % in_url, 'info')
        return None

    try:
        if found_extractor['preloader'] is not None:
            logging.error("Modifying Url: %s" % in_url)
        ...
        logging.error("Probing source: %s" % in_url)
...
def __check_video_list(refer_url, vidlist, add_referer=False,
    ...
    for item in vidlist:
        try:
            ...
            if temp_req[1] != '200':
                logging.error("[*] Skiping Invalid Url: %s - status = %d" % (item[1], temp_req.status_code))
                continue  # Skip Item.
            ...
            logging.error('Error when checking: {0}'.format(e))
            pass
