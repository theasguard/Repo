import abc
import xbmcgui
from asguard_lib import scraper_utils, salts_utils
from asguard_lib.globals import g
from asguard_lib.resolver.torrent_resolvers.apibase import ApiBase

class FileIdentification(Exception):
    def __init__(self, file_list):
        self.file_list = file_list
        super().__init__(f"File identification failed for files: {file_list}")
...
class TorrentResolverBase(ApiBase):
    ...
    def _filter_non_playable_files(folder_details):
        return [i for i in folder_details if salts_utils.is_file_ext_valid(i["path"])]
    ...
    def _sort_and_filter_files(self, folder_details, item_information, sort=False):
        filtered_files = self._filter_non_playable_files(
            scraper_utils.filter_files_for_resolving(folder_details, item_information)
        )
    ...
    def _multi_pack_resolve(self, item_information, torrent):
        ...
        best_match = scraper_utils.get_best_episode_match("path", folder_details, item_information)
    ...
    def _movie_resolve(self, item_information, torrent):
        ...
        folder_details = scraper_utils.filter_files_for_resolving(folder_details, item_information)
        filter_list = [
            i
            for i in folder_details
            if scraper_utils.filter_movie_title(
                None,
                i["path"].split("/")[-1],
