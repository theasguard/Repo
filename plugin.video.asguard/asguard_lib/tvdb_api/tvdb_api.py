...
class TheTvDb(object):
    ...
    _token = None
    cache = None
    api_key = '3Y018HW1MIGAKZIG'  # default api key
    days_ahead = 120
    ignore_cache = False
    _close_called = False
    session = None
    ...
    def get_data(self, endpoint, prefer_localized=False, params=None):
        '''Get data from TVDB API with support for query parameters'''
        ...
        if prefer_localized:
            headers['Accept-Language'] = self._addon.getLocalizedLanguage()
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self._log_msg("TVDB: error %s for %s" % (response.status_code, url), xbmc.LOGWARNING)
                return None
        except Exception as e:
            self._log_msg("TVDB: error in get_data: %s" % str(e), xbmc.LOGERROR)
            return None
    ...
    def get_episode(self, episodeid, seriesdetails=None):
        '''get episode details by episodeid'''
        if not episodeid:
            return None
        episode_details = self.get_data('episodes/%s' % episodeid)
        if not episode_details:
            return None
        return self._map_episode_data(episode_details, seriesdetails)

    @use_cache(14)
    def get_seasons(self, seriesid):
        """Get all seasons for a series"""
        if not seriesid:
            return None
        seasons_data = self.get_data(f'series/{seriesid}/episodes/summary')
        return seasons_data.get('data', {}).get('airedSeasons', []) if seasons_data else None

    @use_cache(7)
    def get_episodes_by_season(self, seriesid, season):
        """Get all episodes for a specific season (TVDB v3)"""
        if not seriesid or not season:
            return None
            
        # TVDB v3 endpoint for season query
    ...
    def _get_token(self, refresh=False):
        ...
        if self._token and not refresh:
            return self._token
        token = self._win.getProperty("plugin.video.asguard.token")
        if token and not refresh:
            return token

        # refresh previous token
        prev_token = self._addon.getSetting("tvdbtokenov")
        if prev_token:
            ...
            if response and response.content and response.status_code == 200:
                ...
                token = data["token"]
            if token:
                self._win.setProperty("plugin.video.asguard.token", token)
                self._token = token
                return token
        ...
        if response and response.content and response.status_code == 200:
            data = json.loads(response.content)
            token = data["token"]
            self._addon.setSetting("tvdbtokenov", token)
            self._win.setProperty("plugin.video.asguard.token", token)
            self._token = token
            return token
        else:
